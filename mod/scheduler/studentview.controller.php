...
function scheduler_book_slot($scheduler, $slotid, $userid, $groupid, $mform, $formdata, $returnurl, $var = 0 ) {
    ...
    foreach ($userstobook as $studentid) {
        ...
        if($var == 1){
            $a = $DB->get_record('scheduler_appointment',['slotid' => $slotid]);
            $DB->update_record_raw('scheduler_appointment',['id' =>$a->id ,'isonline'=>1]);
        }
...
if ($action == 'bookslot') {
    ...
    scheduler_book_slot($scheduler, $slotid, $USER->id, $appointgroup, null, null, $returnurl,$var);
